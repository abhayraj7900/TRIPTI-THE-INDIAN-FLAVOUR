...
export async function POST(request: Request) {
  try {
    ...
    const phone = normalizePhone(
      session?.phone ?? (staff ? (input.phone ?? '') : ''),
    );
    ...
    if (
      ...
      !bookingTime ||
      !tableNumber ||
      !Number.isFinite(requestedGuests) ||
      requestedGuests < 1 ||
      tableIndex < 1 ||
      tableIndex > 16
    ) {
    ...
    const conflict = await db
      ...
      .bind(tableNumber, bookingDate, bookingTime)
      .first();
    if (conflict)
      return Response.json(
    ...
    await db
      ...
      .run();
    let whatsappStatus: 'sent' | 'failed' | 'setup_required' = 'setup_required';
    if (env.WHATSAPP_BOOKING_WEBHOOK_URL) {
      try {
        const notification = await fetch(env.WHATSAPP_BOOKING_WEBHOOK_URL, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(env.WHATSAPP_BOOKING_WEBHOOK_SECRET
              ? {
                  Authorization: `Bearer ${env.WHATSAPP_BOOKING_WEBHOOK_SECRET}`,
                }
              : {}),
          },
          body: JSON.stringify({
            ...
            bookingTime,
            guests,
            message: `Tripti booking ${bookingNumber} is confirmed for table ${tableNumber} on ${bookingDate} at ${bookingTime} for ${guests} guest${guests === 1 ? '' : 's'}.`,
          }),
        });
        whatsappStatus = notification.ok ? 'sent' : 'failed';
      } catch {
...
export async function PATCH(request: Request) {
  try {
    ...
    const result = await db
      ...
      .bind(input.status, Date.now(), input.id)
      .run();
