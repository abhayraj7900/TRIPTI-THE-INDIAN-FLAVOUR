...
export function CustomerAccount() {
  ...
  async function login() {
    setError('');
    const response = await fetch('/api/auth/customer', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone, pin }) });
...
function BookingCard({ onBooked }: { onBooked: () => Promise<void> }) {
  ...
  async function book() {
    ...
    const response = await fetch('/api/bookings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customerName, guests, bookingDate, bookingTime, tableNumber, notes }) });
  ...
  return <div className="h-fit rounded-[26px] border border-[#dfd3c9] bg-white p-6 shadow-sm"><span className="grid size-12 place-items-center rounded-2xl bg-[#fff0d4] text-[#7c310e]"><CalendarDays /></span><h2 className="mt-4 font-serif text-2xl font-black">Book a table</h2><p className="mt-1 text-sm leading-5 text-[#7b655b]">Your booking and order history stays attached to your signed-in mobile number.</p><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2"><label className="space-y-2 text-sm font-bold sm:col-span-2 lg:col-span-1 xl:col-span-2">Booking name<Input value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Name for the table" /></label><label className="space-y-2 text-sm font-bold">Guests<div className="relative"><Users className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#8b776d]" /><Input type="number" min="1" max="20" value={guests} onChange={(event) => setGuests(Number(event.target.value))} className="pl-9" /></div></label><label className="space-y-2 text-sm font-bold">Table 01–16<Input value={tableNumber} onChange={(event) => setTableNumber(event.target.value.replace(/\D/g, '').slice(0, 2))} /></label><label className="space-y-2 text-sm font-bold">Date<Input type="date" min={new Date().toISOString().slice(0, 10)} value={bookingDate} onChange={(event) => setBookingDate(event.target.value)} /></label><label className="space-y-2 text-sm font-bold">Time<Input type="time" value={bookingTime} onChange={(event) => setBookingTime(event.target.value)} /></label><label className="space-y-2 text-sm font-bold sm:col-span-2 lg:col-span-1 xl:col-span-2">Special request<Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Birthday, high chair, accessibility…" /></label></div>{message && <p className={`mt-4 rounded-xl p-3 text-sm font-bold ${message.includes('confirmed') ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p>}<Button onClick={book} disabled={saving || !customerName.trim() || !bookingDate || !bookingTime || !tableNumber} className="mt-5 h-11 w-full bg-[#6a2116] font-bold hover:bg-[#521008]">{saving ? 'Booking…' : 'Confirm table booking'}</Button></div>;
}
