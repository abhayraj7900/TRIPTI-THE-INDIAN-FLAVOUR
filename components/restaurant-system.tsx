...
function OrderDetailsDialog({ order, catalog, onOpenChange, onUpdate, onDelete }: { order: OrderRecord | null; catalog: MenuItem[]; onOpenChange: (open: boolean) => void; onUpdate: (order: OrderRecord) => Promise<boolean>; onDelete: (order: OrderRecord) => Promise<boolean> }) {
  ...
  function addDish() {
    ...
    setDraft((current) => {
      ...
      return { ...current, items: exists ? items.map((item) => item.menuItemId === dish.id ? { ...item, quantity: item.quantity + 1 } : item) : [...items, { menuItemId: dish.id, name: dish.name, quantity: 1, unitPrice: dish.price }] };
    });
  }
  async function save() {
    if (!draft) return;
